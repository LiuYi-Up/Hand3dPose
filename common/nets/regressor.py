...
class Regressor(nn.Module):
    def __init__(self):
        ...
        self.gatconv1 = GATConv(235, 32, 4)
        self.gatconv2 = GATConv(128, 64)
        self.gconv_output = ChebConv(in_c=64, out_c=3, K=2)
    ...
    def forward(self, feature_s, target_s):
        ...
        x = self.gatconv1(batch.x, batch.edge_index)
        x = self.leakyrelu(x)
        x = self.gatconv2(x, batch.edge_index).reshape(B, N, -1)
