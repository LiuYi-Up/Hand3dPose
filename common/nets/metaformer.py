...
# from utils.ChebConv import ChebConv
...
class MetaFormer(nn.Module):
    ...
    def __init__(self, layers, embed_dims=None, 
        ...
        #     else nn.Identity()
        self.head = ConvModule(embed_dims[-1], 512+21, 1, norm_cfg=dict(type='BN'))
...
def make_metaformer():
    ...
    layers = [2, 2]
    embed_dims = [256, 512]  # 先把各个参数弄明白
    token_mixers = [Pooling, Attention]
    mlp_ratios = [4, 4]
    downsamples = [False, False]
...
if __name__ == '__main__':
    model = make_metaformer()
    print(model)
    import numpy as np
    input = np.random.rand(8, 256, 64, 64)
    out = model(input)